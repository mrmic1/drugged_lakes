...
lake_BT_pike_OUF_models <- lapply(rds_list, function(x) x[['OUF anisotropic error']])

# Set the names of the new list to the identities of the original objects (if needed)
names(lake_BT_pike_OUF_models) <- names(rds_list)
print(names(lake_BT_pike_OUF_models))

#Check model outputs
for (i in 1:length(lake_BT_pike_OUF_models)) {
  element_name <- names(lake_BT_pike_OUF_models)[i]
  cat("Summary for", element_name, ":\n")
  print(summary(lake_BT_pike_OUF_models[[i]]))
  cat("\n")
}

#save list
saveRDS(lake_BT_pike_OUF_models, paste0(ctmm_path, "lake_BT_pike_OUF_models.rds"))
