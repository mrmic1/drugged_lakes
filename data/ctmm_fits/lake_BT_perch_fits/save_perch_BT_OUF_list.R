...
BT_perch_OUF_models <- list()

#Iterate over each object in muddyfoot_roach_ctmm_fits and extract the 'OUF anisotropic error' models
BT_perch_OUF_models <- lapply(BT_perch_ctmm_fits, function(x) x[['OUF anisotropic error']])
#check it worked
summary(BT_perch_OUF_models$F59750)

#rejoin pred filtered models
BT_perch_OUF_models <- c(BT_perch_OUF_models, remaining_list)[names(rds_list)]
