#--------------------------------#
# Combine cow paradise roach ctmms
#--------------------------------#

ctmm_path = "./data/ctmm_fits/lake_cow_roach_fits/"
...
lake_cow_roach_OUF_models <- lapply(rds_list, function(x) x[['OUF anisotropic error']])

# Set the names of the new list to the identities of the original objects (if needed)
names(lake_cow_roach_OUF_models) <- names(rds_list)
print(names(lake_cow_roach_OUF_models))

#Check model outputs
for (i in 1:length(lake_cow_roach_OUF_models)) {
  element_name <- names(lake_cow_roach_OUF_models)[i]
  cat("Summary for", element_name, ":\n")
  print(summary(lake_cow_roach_OUF_models[[i]]))
  cat("\n")
}

#Now add F59893 to list
#before adding to list make sure it has the same projection
ctmm::projection(F59819) <- ctmm::projection(lake_cow_roach_OUF_models)

lake_cow_roach_OUF_models <- c(lake_cow_roach_OUF_models, list(F59819 = F59819))
names(lake_cow_roach_OUF_models)
# Reorder the list chronologically by element names
lake_cow_roach_OUF_models <- lake_cow_roach_OUF_models[order(names(lake_cow_roach_OUF_models))]
names(lake_cow_roach_OUF_models)


#save list
saveRDS(lake_cow_roach_OUF_models, paste0(ctmm_path, "lake_cow_roach_OUF_models.rds"))
