#-----------------------------#
# Combine BT roach ctmms
#-----------------------------#

ctmm_path = "./data/ctmm_fits/lake_BT_roach_fits/"
...
exclude_ids <- c("F59776", "F59783", "F59803", "F59807", "F59809", "F59810", "F59815", "F59812")

BT_roach_ctmm_fits <- rds_list[!names(rds_list) %in% exclude_ids]
...
BT_roach_OUF_models <- list()

#Iterate over each object in muddyfoot_roach_ctmm_fits and extract the 'OUF anisotropic error' models
BT_roach_OUF_models <- lapply(BT_roach_ctmm_fits, function(x) x[['OUF anisotropic error']])
#check it worked
summary(BT_roach_OUF_models$F59777)

#rejoin pred filtered models
BT_roach_OUF_models <- c(BT_roach_OUF_models, remaining_list)[names(rds_list)]
#check that ID are in chonological order
names(BT_roach_OUF_models)
summary(BT_roach_OUF_models$F59812)


#save
saveRDS(BT_roach_OUF_models, paste0(ctmm_path, "BT_roach_OUF_models.rds"))
