# =================================================================-
# Encounter Rate Analysis - Muddyfoot ####
# =================================================================-
#
# ANALYSIS OVERVIEW:
...
#
#The above analyses will be conducted on the dataset post filtering to remove tracks following
#predation. Encounter rates in the pre-filtering dataset will be inflated by prey fish
#that were predated
#
#
#=================================================================-
...
paths <- list(
  ...
  filtered   = "./data/tracks_filtered/muddyfoot/",
  mortality  = "./data/encounters/suspected_mortality_updated.xlsx",
  figures    = "./figures/muddyfoot/encounter_analysis/",
  tables     = "./tables/muddyfoot/encounter_analysis/"
)
...
# Calculated for mid-experiment at lake coordinates (lat 63.77, lon 20.05)
# using suncalc::getSunlightTimes(). See 03_daily_activity.Rmd for derivation.
...
# Print clean, labelled, copy-pasteable summaries for manuscript writing.
# Call after every model to produce a self-contained console block that
# can be pasted directly into Claude for interpretation / results writing.
...
roach_pike_distances_df <- readRDS(
  paste0(paths$encounters, "muddyfoot_pike_roach_distances_tiered_df.rds")
)
perch_pike_distances_df <- readRDS(
  paste0(paths$encounters, "muddyfoot_pike_perch_distances_tiered_df.rds")
)

## 2.2 Load metadata ----
muddyfoot_filt_data <- readRDS(
  paste0(paths$filtered, "05_muddyfoot_sub.rds")
)

#extracting ID, treatment and weight information
muddyfoot_meta <- muddyfoot_filt_data %>%
...
muddyfoot_death_dates <- mortality_preds %>%
...
print(table(muddyfoot_death_dates$species,
            muddyfoot_death_dates$revised_suspected_mortality))

## 3.2 Filter roach-pike distance data ----
roach_pike_distances_df <- 
  roach_pike_distances_df %>%
  left_join(
    muddyfoot_death_dates %>% select(individual_ID, death_date),
...
# weight_z is scaled within each species separately so it is relative
# to conspecifics, not pooled across species.
...
enc_roach_total <- roach_pike_distances_df %>%
  ...
  left_join(muddyfoot_meta, by = "individual_ID") %>%
...
# Aggregated to individual x date x hour, then joined with metadata.
...
# Rates for predated individuals will be inflated; included to allow
# direct comparison with the post-filtered results in section 5.

message("\n=== Q1 PRE-FILTERING ANALYSIS (MUDDYFOOT) ===")
...
## 4.5.2 Load pre-filtered metadata (04_muddyfoot_sub.rds) ----
muddyfoot_pre_data <- readRDS(
  paste0(paths$filtered, "04_muddyfoot_sub.rds")
)

muddyfoot_pre_meta <- muddyfoot_pre_data %>%
  select(individual_ID, treatment, weight) %>%
  distinct()
...
enc_roach_pre <- roach_pike_raw %>%
  ...
  left_join(muddyfoot_pre_meta, by = "individual_ID") %>%
  filter(!is.na(treatment)) %>%
  mutate(
    Species        = "Roach",
    rate_high_conf = total_encounters_high_conf / n_days,
...
message("\nPre-filter encounter rate summary — Muddyfoot:")
...
print_glmm_results(enc_mod_roach_hc_pre,
                   ...
                   species      = "Roach",
                   response_var = "total_encounters_high_conf")
...
#
#   (A) HIGH-CONFIDENCE ONLY: encounters <= 1.4m (within 95% combined
#       GPS error). The most conservative and spatially precise tier.
#
#   (B) COMBINED (high-confidence + probable): encounters <= 2.8m.
#       Includes the probable tier (1.4-2.8m) to capture encounters that
#       are real but fall just outside the strictest GPS error threshold.
#       Running both allows assessment of whether treatment effects are
#       robust across encounter definitions or sensitive to the threshold.
#
# This is a single-lake study (Muddyfoot), so lake is not included as a
# factor. Individual is included as a random intercept to account for
# variation in baseline encounter rates.
...
enc_mod_roach_hc <- glmmTMB(
  ...
  family = nbinom2(link = "log"),
  offset = log(n_days)   # Converts count to daily rate
)
...
print_glmm_results(enc_mod_roach_comb,
                   ...
                   question     = "Q1 Post-filter",
                   species      = "Roach",
                   response_var = "total_encounters_combined")
message("  Marginal means (encounters/day):")
print(emm_roach_comb)
message("  Pairwise contrast (Mix vs Control):")
print(pairs(emm_roach_comb))
...
# Two plots produced: one per encounter definition. Both use free y-scales
# so roach and perch are not forced onto the same axis despite differing
# baseline encounter rates. Panels are arranged for direct comparison.
...
p_enc_hc <- ggplot(
  ...
  theme(
    ...
    # Enclose each panel in a box
    panel.border     = element_rect(colour = "black", fill = NA)
  )

ggsave(file.path(paths$figures, "encounter_rates_high_conf.png"),
...
ggsave(file.path(paths$figures, "encounter_rates_combined.png"),
...
# =================================================================-
# Adapted from 03_daily_activity.Rmd. Key differences: response is
# n_encounters_high_conf (integer counts), family = nb() not Gamma().
# Single-lake study (Muddyfoot), so lake is not a factor. One model
# per species, run across all individuals together.

message("\n=== Q2/Q3: DIEL ENCOUNTER PATTERN GAMMs ===")

## 6.1 Prepare hourly datasets ----
# Single experiment start date for Muddyfoot. Individual random effects
# account for among-individual variation in diel encounter patterns.

roach_h <- roach_hourly %>%
  mutate(
    exp_day = as.integer(
      as.duration(as.POSIXct(paste(Date, "00:00:00")) -
                    as.POSIXct("2022-09-25")) / lubridate::ddays(1)
    ),
    treatment     = factor(treatment, levels = c("Control", "Mix")),
    individual_ID = factor(individual_ID),
...
# Required by bam() to mark the first row of each individual's time series
# so the AR1 process resets between individuals rather than carrying over.
...
# Fit a first-pass model without AR correction, extract per-individual
# lag-1 ACF from Pearson residuals, then average across all individuals
# to get the single rho value passed to the final AR-corrected model.
# Mirrors the ACF estimation section in 03_daily_activity.Rmd.
...
# enc_colours defined in section 5 — c("Control" = "#004D40", "Mix" = "#D81B60")

add_diel_lines <- function(p) {
  # Vertical dashed lines at nautical dawn, sunrise, sunset, nautical dusk
  p +
...
message("\n--- Q2/Q3: Roach diel encounter GAMM ---")
...
# Cyclic cubic spline (bs="cc") enforces that the encounter curve wraps
# continuously from hour 23 back to hour 0. s(hour, by = treatment)
# estimates separate diel curves per treatment group (Q3). The random
# smooth s(hour, individual_ID, bs="fs") allows each individual to have
# its own diel encounter shape around the population-level curves.
roach_mod <- bam(
  n_encounters_high_conf ~
    treatment + exp_day_z + weight_z +
    s(hour, by = treatment, k = 15, bs = "cc") +
...
print_gamm_results(roach_mod, lake = "MUDDYFOOT", species = "Roach")
...
# Estimated difference (Mix - Control) on the response scale at each hour
# of day. Confidence intervals crossing zero indicate no detectable
# difference at that time. Peaks identify when treatment divergence is
# greatest, e.g. during crepuscular pike hunting activity.
roach_comp <- comparisons(roach_mod, newdata = newdata_roach,
                          variables = "treatment", type = "response",
                          vcov = TRUE, by = "hour")

message("\nRoach — peak/trough treatment difference:")
print(
  roach_comp %>%
...
# Tests whether encounters are highest during dawn/dusk (crepuscular pike
# activity hypothesis). Hours are assigned to four biologically meaningful
# periods using the reference times from section 1.3:
#   Night : hour < nauticalDawn  OR  hour >= nauticalDusk
#   Dawn  : nauticalDawn <= hour < sunrise
#   Day   : sunrise      <= hour < sunset
#   Dusk  : sunset       <= hour < nauticalDusk
#
# Approach:
#   1. Assign periods to the existing prediction grid (newdata_roach).
#   2. Summarise mean model-predicted encounters per period and per
#      period × treatment — gives the descriptive peak pattern.
#   3. avg_predictions() + hypotheses() from marginaleffects provides
#      formal pairwise tests between all period combinations.
...
# Descriptive: mean predicted encounters per period (pooled treatments)
...
roach_period_summary <- roach_period_raw %>%
  ...
  summarise(
    mean_encounters = mean(fitted),
    lower_95ci      = mean(lower),
    upper_95ci      = mean(upper),
    .groups = "drop"
  ) %>%
  arrange(desc(mean_encounters))

message("\nRoach — mean predicted encounters by diel period (pooled treatments):")
print(roach_period_summary)

# Descriptive: per treatment breakdown
...
# Formal pairwise contrasts between diel periods using marginaleffects:
# avg_predictions() computes the marginal mean per period across all
# individuals and both treatments; hypotheses() tests all pairwise
# differences and returns p-values and 95% CIs.
roach_period_preds <- avg_predictions(
  roach_mod,
  newdata = newdata_roach_periods,
  by      = "diel_period",
  type    = "response"
)

message("\nRoach — marginal mean encounters per diel period:")
print(roach_period_preds)

# Pairwise contrasts: b1=Night, b2=Dawn, b3=Day, b4=Dusk
# (order matches levels = c("Night","Dawn","Day","Dusk"))
roach_period_pairs <- hypotheses(
  roach_period_preds,
  hypothesis = c(
    "b1 - b2 = 0",   # Night vs Dawn
    "b1 - b3 = 0",   # Night vs Day
    "b1 - b4 = 0",   # Night vs Dusk
    "b2 - b3 = 0",   # Dawn  vs Day
    "b2 - b4 = 0",   # Dawn  vs Dusk
    "b3 - b4 = 0"    # Day   vs Dusk
  )
)

message("\nRoach — pairwise diel period contrasts:")
print(roach_period_pairs)

## 6.10 Roach — diel encounter figure ----
# Left panel: predicted encounter curves per treatment (Q2).
# Right panel: treatment contrast (Mix - Control) with 95% CI (Q3).
# Vertical dashed lines: nautical dawn, sunrise, sunset, nautical dusk.
roach_diel_plot     <- plot_diel_enc(roach_pred_summary, "Roach")
roach_contrast_plot <- plot_enc_contrast(roach_comp, "Roach")

roach_figure <- ggarrange(roach_diel_plot, roach_contrast_plot,
                          labels = c("A", "B"), ncol = 2)

ggsave(file.path(paths$figures, "roach_diel_encounter_figure.png"),
       roach_figure, width = 12, height = 5, dpi = 300)
message("Saved: roach_diel_encounter_figure.png")

# ----------------------------------------------------------------
# PERCH GAMM
# ----------------------------------------------------------------

message("\n--- Q2/Q3: Perch diel encounter GAMM ---")
...
print_gamm_results(perch_mod, lake = "MUDDYFOOT", species = "Perch")
...
message("\nPerch — mean predicted encounters by diel period (pooled treatments):")
...
perch_contrast_plot <- plot_enc_contrast(perch_comp, "Perch")

perch_figure <- ggarrange(perch_diel_plot, perch_contrast_plot,
                          labels = c("A", "B"), ncol = 2)

ggsave(file.path(paths$figures, "perch_diel_encounter_figure.png"),
       perch_figure, width = 12, height = 5, dpi = 300)
message("Saved: perch_diel_encounter_figure.png")
...
print(p_enc_diel_overview)

ggsave(file.path(paths$figures, "muddyfoot_combined_diel_encounter_overview.png"),
...
cat("Encounter Rate Analysis Session Info\n")
