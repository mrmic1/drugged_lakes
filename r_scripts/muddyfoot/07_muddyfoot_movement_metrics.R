...
muddyfoot_track_data <-  readRDS(paste0(filtered_data_path, "04_muddyfoot_sub.rds"))
...
muddyfoot_track_data <- 
  muddyfoot_track_data %>% 
  dplyr::select(-speed, -distance, -first_date_over_50,  -VAR.xy, -x, -y)
...
muddyfoot_track_data <- 
  muddyfoot_track_data %>% 
  select(individual_ID, Species, Treatment, Weight, Total_length, Std_length, 
         timestamp, Time_Of_Day, Date, Exp_Stage, Long, Lat, time_diff, 
         distance_m, speed_cm_s, turnangle, azimuth, everything())


#save
saveRDS(muddyfoot_track_data, paste0(filtered_data_path, "05_muddyfoot_sub.rds"))
