#===============================================================================
# COW PARADISE LAKE - FISH HOME RANGE ANALYSIS WITH AKDE AND BOUNDARY CORRECTION
#===============================================================================
#
# PURPOSE:
...
tables_path  <- "./tables/cow_paradise/"
...
dir.create(paste0(akde_path, "cow_paradise_pike_akdes/"),  showWarnings = FALSE)
dir.create(paste0(akde_path, "cow_paradise_perch_akdes/"), showWarnings = FALSE)
dir.create(paste0(akde_path, "cow_paradise_roach_akdes/"), showWarnings = FALSE)
...
message("Pike:  ", length(pike_cow_tel),  " individuals")
message("Perch: ", length(perch_cow_tel), " individuals")
message("Roach: ", length(roach_cow_tel), " individuals")

# ctmm fits
pike_cow_paradise_ctmm_fits  <- readRDS(paste0(ctmm_path, "cow_paradise_pike_fits/cow_paradise_pike_best_models.rds"))
perch_cow_paradise_ctmm_fits <- readRDS(paste0(ctmm_path, "cow_paradise_perch_fits/cow_paradise_perch_best_models.rds"))
roach_cow_paradise_ctmm_fits <- readRDS(paste0(ctmm_path, "cow_paradise_roach_fits/cow_paradise_roach_best_models.rds"))

# Lake boundary
cow_paradise_polygon <- st_read(paste0(polygon_path, "cow_polygon.gpkg"), quiet = TRUE)
cow_paradise_sp_data <- as(cow_paradise_polygon, "Spatial")

message("\nPolygon CRS: ", st_crs(cow_paradise_polygon)$input, "\n")
...
for(species in c("perch", "roach", "pike")) {
  tel  <- get(paste0(species, "_cow_tel"))
  fits <- get(paste0(species, "_cow_paradise_ctmm_fits"))
...
run_species_akde <- function(tel_list,
  ...
  saveRDS(akde_list, paste0(akde_save_path, tolower(species_name), "_cow_paradise_akdes.rds"))
...
pike_results <- run_species_akde(
  tel_list      = pike_cow_tel,
  fits_list     = pike_cow_paradise_ctmm_fits,
  sp_data       = cow_paradise_sp_data,
  control_idx   = 1:3,
  exposed_idx   = 4:6,
  species_name  = "Pike",
  akde_save_path = paste0(akde_path, "cow_paradise_pike_akdes/"),
  min_N         = 3
)

pike_cow_paradise_akdes  <- pike_results$akdes
...
perch_results <- run_species_akde(
  tel_list      = perch_cow_tel,
  fits_list     = perch_cow_paradise_ctmm_fits,
  sp_data       = cow_paradise_sp_data,
  control_idx   = 1:15,
  exposed_idx   = 16:30,
  species_name  = "Perch",
  akde_save_path = paste0(akde_path, "cow_paradise_perch_akdes/"),
  min_N         = 3
)


perch_cow_paradise_akdes <- perch_results$akdes
...
roach_results <- run_species_akde(
  tel_list      = roach_cow_tel,
  fits_list     = roach_cow_paradise_ctmm_fits,
  sp_data       = cow_paradise_sp_data,
  control_idx   = 1:13,
  exposed_idx   = 14:26,
  species_name  = "Roach",
  akde_save_path = paste0(akde_path, "cow_paradise_roach_akdes/"),
  min_N         = 3
)

roach_cow_paradise_akdes <- roach_results$akdes
...
write.csv(summary_data,
          paste0(tables_path, "cow_paradise_home_range_summary.csv"),
          row.names = FALSE)
