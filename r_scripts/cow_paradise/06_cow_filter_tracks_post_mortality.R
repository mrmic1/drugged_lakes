...
# Load data
cow_paradise_telem_data <- readRDS(paste0(filtered_data_path, "04_cow_sub.rds"))
mortality_preds <- readxl::read_excel("./data/encounters/suspected_mortality_updated.xlsx")
message("\nOriginal dataset: ", nrow(cow_paradise_telem_data), " rows")
pike_mortality <- read.csv("./data/pike_deaths.csv")

# Prepare mortality data
cow_paradise_pred_prey_cols <- mortality_preds %>%
...
cow_paradise_pred_prey_cols$death_date <- as.Date(cow_paradise_pred_prey_cols$death_date, origin = "1970-01-01")

message("\nIndividuals with mortality events:")
# Get species info from original data for summary
mortality_summary <- 
  cow_paradise_telem_data %>%
  filter(individual_ID %in% cow_paradise_pred_prey_cols$individual_ID) %>%
  distinct(individual_ID, species) %>%
  left_join(cow_paradise_pred_prey_cols, by = "individual_ID")
...
cow_paradise_telem_data_2 <- cow_paradise_telem_data %>%
  left_join(cow_paradise_pred_prey_cols %>% select(individual_ID, death_date), 
            by = "individual_ID") %>%
  filter(is.na(death_date) | date < death_date) %>%
  select(-death_date)  # Remove the death_date column after filtering

rows_removed <- nrow(cow_paradise_telem_data) - nrow(cow_paradise_telem_data_2)
message("\nRows removed after filtering: ", format(rows_removed, big.mark = ",")) #Rows removed after filtering: 42,017
message("Filtered dataset: ", nrow(cow_paradise_telem_data_2), " rows") #1921283 rows

#Now I need to filter out pike mortality
cow_paradise_pike_mort <- pike_mortality %>%
  filter(lake == 'Cow Paradise') %>% 
  dplyr::select(individual_ID, likely_death_date) %>%  # Remove species here
  rename(death_date = likely_death_date)

# Ensure death_date is Date type
cow_paradise_pike_mort$death_date <- as.Date(cow_paradise_pike_mort$death_date, origin = "1970-01-01")

# Filter out data after the predation or mortality event
cow_paradise_telem_data_3 <- cow_paradise_telem_data_2 %>%
  left_join(cow_paradise_pike_mort %>% select(individual_ID, death_date), 
            by = "individual_ID") %>%
  filter(is.na(death_date) | date < death_date) %>%
  select(-death_date)  # Remove the death_date column after filtering

rows_removed <- nrow(cow_paradise_telem_data_2) - nrow(cow_paradise_telem_data_3)
message("\nRows removed after filtering: ", format(rows_removed, big.mark = ",")) #Rows removed after filtering: 26,895
message("Filtered dataset: ", nrow(cow_paradise_telem_data_3), " rows") #1894388 rows


# Save filtered data
saveRDS(cow_paradise_telem_data_3, paste0(filtered_data_path, "05_cow_sub.rds"))
...
message("Projection: ", ctmm::projection(cow_paradise_tels[[1]]))
...
saveRDS(pike_cow_paradise_tel, paste0(save_telem_path, "cow_paradise/pike_cow_paradise_tel_thinned_final.rds"))
saveRDS(perch_cow_paradise_tel, paste0(save_telem_path, "cow_paradise/perch_cow_paradise_tel_thinned_final.rds"))
saveRDS(roach_cow_paradise_tel, paste0(save_telem_path, "cow_paradise/roach_cow_paradise_tel_thinned_final.rds"))
...
pike_cow_paradise_tel <- readRDS(paste0(save_telem_path, "cow_paradise/pike_cow_paradise_tel_thinned_final.rds"))
...
# Option to reload if needed ------------------------------------------------
perch_cow_paradise_tel <- readRDS(paste0(save_telem_path, "cow_paradise/perch_cow_paradise_tel_thinned_final.rds"))

if (length(perch_cow_paradise_tel) > 0) {
  # Fit models using parallel processing -----------------------------------
  perch_cores <- ifelse(length(perch_cow_paradise_tel) > 15, 15, 3)
...
# Option to reload if needed ------------------------------------------------
roach_cow_paradise_tel <- readRDS(paste0(save_telem_path, "cow_paradise/roach_cow_paradise_tel_thinned_final.rds"))

if (length(roach_cow_paradise_tel) > 0) {
  # Fit models using parallel processing -----------------------------------
  roach_cores <- ifelse(length(roach_cow_paradise_tel) > 15, 20, 3)
