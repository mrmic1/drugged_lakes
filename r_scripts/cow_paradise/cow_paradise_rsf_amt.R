...
rsf_output_path <- "./output/cow_paradise/rsf/"
...
cat("\n=== DATA SUMMARY ===\n")
cat("Timestamp range:", format(range(cow_paradise_filt_data$timestamp)), "\n")
cat("Number of individuals:", n_distinct(cow_paradise_filt_data$individual_ID), "\n")
cat("Species:", paste(unique(cow_paradise_filt_data$species), collapse = ", "), "\n")
cat("Total observations:", nrow(cow_paradise_filt_data), "\n")

# Convert to track format and project to UTM 33N -------------------------------
cow_paradise_trk <- cow_paradise_filt_data |>
  transmute(
    x_ = Long,
    y_ = Lat,
    t_ = timestamp,
...
class(cow_paradise_trk) <- c("track_xyt", "track_xy", "tbl_df", "tbl", "data.frame")
attr(cow_paradise_trk, "crs_") <- st_crs(32633)
...
cow_paradise_trk_resampled <- cow_paradise_trk |>
...
# Summary
cat("Data retention:\n")
print(cow_paradise_trk_resampled |> 
...
cow_hab_locs <- st_read(cow_rec_locs_kml, quiet = TRUE)[c(2,3,5,7), ]
cow_paradise_polygon <- st_read(paste0(polygon_path, "cow_polygon.gpkg"), quiet = TRUE)

# Transform to UTM 33N
cow_hab_locs <- st_transform(cow_hab_locs, crs = 32633)
cow_paradise_polygon <- st_transform(cow_paradise_polygon, crs = 32633)

# Buffer habitats to account for telemetry error ------------------------------
cat("Buffering habitat patches by 0.5m for telemetry error...\n")

cow_hab_locs_buffered <- st_buffer(cow_hab_locs, dist = 0.5)
...
lake_extent <- ext(cow_paradise_polygon)

# Create raster template
lake_raster <- rast(lake_extent, res = resolution, crs = "EPSG:32633")
lake_raster <- rasterize(vect(cow_paradise_polygon), lake_raster, field = 1, background = NA)

# Create binary habitat raster (buffered)
habitat_raster <- rasterize(
  vect(cow_hab_locs_buffered),
  lake_raster,
  field = 1,
  background = 0
)
habitat_raster <- mask(habitat_raster, vect(cow_paradise_polygon))

# Better visualization to show buffer vs original
par(mfrow = c(1, 2))

# Plot 1: Original vs Buffered polygons
plot(st_geometry(cow_paradise_polygon), main = "Original (blue) vs Buffered (red)", 
     col = "lightgray", border = "black")
plot(st_geometry(cow_hab_locs), col = "blue", add = TRUE, lwd = 2)
plot(st_geometry(cow_hab_locs_buffered), col = "red", add = TRUE, lwd = 2, border = "red")
...
plot(st_geometry(cow_paradise_polygon), add = TRUE, lwd = 2)
plot(st_geometry(cow_hab_locs), col = "blue", add = TRUE, lwd = 2)
text(st_coordinates(st_centroid(cow_hab_locs))[,1], 
     st_coordinates(st_centroid(cow_hab_locs))[,2],
     labels = c("Habitat 1", "Habitat 2", "Habitat 3", "Habitat 4"), cex = 0.8)

par(mfrow = c(1, 1))

# Verify buffer dimensions
cat("\nHabitat dimensions:\n")
original_bbox <- st_bbox(cow_hab_locs)
buffered_bbox <- st_bbox(cow_hab_locs_buffered)
...
cow_paradise_trk_locs <- cow_paradise_trk_resampled |>
  select(id, species, treatment, data_filtered) |>
  unnest(cols = data_filtered) |>
  select(x_, y_, t_, id, species, treatment)

# Restore track class
class(cow_paradise_trk_locs) <- c("track_xyt", "track_xy", "tbl_df", "tbl", "data.frame")
attr(cow_paradise_trk_locs, "crs_") <- st_crs(32633)

cat("Total locations:", nrow(cow_paradise_trk_locs), "\n")
cat("Individuals:", n_distinct(cow_paradise_trk_locs$id), "\n")
...
cow_paradise_rsf_nested <- cow_paradise_trk_locs |>
  nest(data = -c(id, species, treatment)) |>
  mutate(
    rsf_data = map(
      data, 
      ~ prepare_rsf_data(.x, cow_paradise_polygon, habitat_raster)
    ),
    n_used = map_int(data, nrow),
    n_available = map_int(rsf_data, ~ sum(.x$case_ == FALSE))
  )

cat("RSF data prepared for", nrow(cow_paradise_rsf_nested), "individuals\n")
print(cow_paradise_rsf_nested |> select(id, species, treatment, n_used, n_available))

# Fit RSF models --------------------------------------------------------------
cat("\n=== FITTING RSF MODELS ===\n")

rsf_results <- cow_paradise_rsf_nested |>
  mutate(
