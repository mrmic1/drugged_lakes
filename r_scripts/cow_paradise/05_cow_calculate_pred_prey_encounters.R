# ===================================================================
# Predator-Prey Interaction Analysis - Lake Cow Paradise
# ===================================================================
# 
# ANALYSIS OVERVIEW:
...
pike_tel <- readRDS(file.path(paths$telem, 'pike_cow_paradise_tel.rds'))
perch_tel <- readRDS(file.path(paths$telem, 'perch_cow_paradise_tel.rds'))
roach_tel <- readRDS(file.path(paths$telem, 'roach_cow_paradise_tel.rds'))

## 3.2 Load CTMM Fits ----
pike_fits <- readRDS(file.path(paths$ctmm, "cow_paradise_pike_fits/cow_paradise_pike_ctmm_fits.rds"))
perch_fits <- readRDS(file.path(paths$ctmm, "cow_paradise_perch_fits/cow_paradise_perch_ctmm_fits.rds"))
roach_fits <- readRDS(file.path(paths$ctmm, "cow_paradise_roach_fits/cow_paradise_roach_ctmm_fits.rds"))

## 3.3 Load Metadata ----
cow_paradise_filt_data <- readRDS(file.path(paths$filtered_data, "03_cow_paradise_sub.rds"))
...
roach_pike_file <- file.path(paths$encounters, "cow_paradise_pike_roach_distances_df.rds")
...
perch_pike_file <- file.path(paths$encounters, "cow_paradise_pike_perch_distances_df.rds")
...
post_biometric_cols <- post_biometrics %>%
  filter(Lake == 'cow_paradise', Species %in% c('Roach', 'Perch')) %>%
...
saveRDS(suspected_predation, 
        file.path(paths$encounters, "cow_paradise_suspected_predation_events.rds"))
...
# Prepare metadata
prey_metadata <- cow_paradise_filt_data %>%
...
saveRDS(prey_encounter_summary,
        file.path(paths$encounters, "cow_paradise_prey_total_encounter_summary.rds"))
...
saveRDS(prey_encounters_filtered,
        file.path(paths$encounters, "cow_paradise_prey_encounters_filtered.rds"))
saveRDS(prey_summary_filtered,
        file.path(paths$encounters, "cow_paradise_prey_encounter_summary_filtered.rds"))
...
if (nrow(suspected_predation) > 0) {
  ...
  ts_plot <- ggplot(time_series_data, aes(x = Date)) +
    ...
    labs(
      ...
      sucow_paradiseitle = sprintf("Species: %s | Consecutive days >100 encounters: %d",
...
# Save workbook
saveWorkbook(wb, 
             file = file.path(paths$tables, "cow_paradise_predation_analysis_results.xlsx"),
             overwrite = TRUE)
