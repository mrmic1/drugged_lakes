...
paths <- list(
  ...
  proximity     = "./data/proximity/cow_paradise/",
...
# Combined GPS error for prey+predator: √(0.37² + 0.37²) = 0.52m
# 95% CI: ~1.4m
# Combined GPS error for prey+predator: √(0.827² + 0.827²) = 1.17m
# 95% CI: ~3.0m (using 2.6σ for 95% confidence)
params <- list(
  # GPS error for Cow Paradise lake (from UERE analysis)
  gps_error_sd = 0.827,  # Updated from 0.379m to 0.827m
  
  # Combined GPS error for prey+predator: √(0.827² + 0.827²) = 1.17m
  # 95% CI: ~3.0m (using 2.6σ for 95% confidence)
  
  # Tiered distance thresholds (meters) - UPDATED FOR COW PARADISE
  strike_distance = 0.45,              # Pike max strike distance (unchanged)
  high_confidence_threshold = 3.0,     # ≤3.0m: Within 95% combined GPS error (≈2.6 × 1.17)
  probable_threshold = 6.0,            # ≤6.0m: 2x GPS error
  possible_threshold = 9.0,            # ≤9.0m: 3x GPS error
  ...
  min_distance_threshold = 3.0,        # Min distance for high-confidence (updated)
  ...
  # Mortality detection settings - UPDATED FOR COW PARADISE
  gps_error_sd = 0.827,                # Cow Paradise-specific GPS error (meters)
...
message("Tiered approach based on GPS error (σ = 0.827m per fish, Cow Paradise lake):")
message(sprintf("  Combined GPS error: %.3fm", sqrt(2) * params$gps_error_sd))
...
message("\n⚠️  NOTE: Higher GPS error (0.827m) requires more conservative thresholds")
message("  Expected GPS noise: ~80m/day (vs ~30m/day for BT lake)")
message("  Mortality threshold: ~200m/day (vs ~75m/day for BT lake)")
message("=====================================\n")
...
identify_predation_events <- function(distances_df,
  ...
  predation_summary <- high_risk_days %>%
    ...
    summarise(
      ...
      num_days_min_dist_less_1.5m = sum(daily_min_dist < 3.5),  # Adjusted for higher GPS error
      num_days_min_dist_less_0.5m = sum(daily_min_dist < 1.5),  # Adjusted for higher GPS error
...
plot_distance_timeseries <- function(distances_df,
  ...
  p <- ggplot(pair_data, aes(x = timestamp, y = est)) +
    ...
    labs(
      ...
      subtitle = sprintf(  # Changed from sucowitle
    ...
    theme(
      ...
      plot.sucowitle = element_text(size = 9, color = "gray30"),
  ...
  if (threshold_lines) {
    p <- p +
      ...
      annotate("text", x = min(pair_data$timestamp), y = 3.0,
               label = "High-conf (3.0m)", hjust = 0, vjust = -0.5,
               size = 3, color = "#d73027") +
      annotate("text", x = min(pair_data$timestamp), y = 6.0,
               label = "Probable (6.0m)", hjust = 0, vjust = -0.5,
               size = 3, color = "#fee090")
  }
...
roach_pike_distances_df <- readRDS(paste0(paths$encounters, "cow_pike_roach_distances_tiered_df.rds"))
perch_pike_distances_df <- readRDS(paste0(paths$encounters, "cow_pike_perch_distances_tiered_df.rds"))
...
saveRDS(total_encounters_ID, paste0(paths$encounters, "cow_total_encounters_ID.rds"))
...
p3 <- ggplot(combined_distances %>%
  ...
  labs(
    ...
    sucowitle = sprintf("Thresholds: %.1fm (high-conf), %.1fm (probable)",
                       params$high_confidence_threshold, params$probable_threshold),
    x = "Distance (m)",
    y = "Count",
    fill = "Encounter Type"
...
if (params$run_proximity_analysis && exists("proximity_with_encounters")) {
  ...
  p4 <- ggplot(proximity_with_encounters,
    ...
    labs(
      ...
      sucowitle = "Ratio < 1: closer than expected | Ratio > 1: farther than expected",
...
#Save workbook
saveWorkbook(wb,
             file.path(paths$tables, "cow_predation_analysis_complete.xlsx"),
             overwrite = TRUE)
...
message("\nKey files to review:")
message("  - cow_predation_analysis_complete.xlsx")
