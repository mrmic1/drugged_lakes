# ---------------------------------------------------# 
# Exploring predator-prey interactions - Cow Paradise
# ---------------------------------------------------#

# This script analyses telemetry data of prey (Roach, Perch) and predators (Pike)
# in cow paradise, including pairwise distances and encounter rates.
...
saveRDS(roach_pike_distances_df, paste0(enc_path, "cow_pike_roach_distances_df.rds"))
...
over_50_encounters <- 
  roach_pike_encounter_sum %>%
  filter(encounter_count >= 50 | daily_avg_dist < 3| Known_predated)
...
saveRDS(cow_roach_pike_pred_int, paste0(enc_path, "cow_roach_predation_events.rds"))
...
post_size_cols <- 
  post_biometrics %>%
  filter(Lake == 'Cow Paradise', Species == 'Perch') %>%
  dplyr::select(individual_ID, Found, Known_predated) %>% 
  rename(found_alive = Found)
...
cow_perch_pike_pred_int <- 
  ...
  summarise(
    ...
    daily_avg_dist = mean(daily_avg_dist)
  )
...
cow_perch_pike_pred_int <-
  ...
  mutate(likely_predated = ifelse(consecutive_days >= 1 | Known_predated == 1, 1, 0))
...
saveRDS(cow_perch_pike_pred_int, paste0(enc_path, "cow_perch_pike_pred_int.rds")) 

#----------------------------------------------#
# 3. Identify predation events in cow ####
#----------------------------------------------#

# Combine predation events into a single dataframe
cow_pred_events <- 
  rbind(
    cow_roach_pike_pred_int %>% rename(individual_ID = Roach_ID) %>% mutate(Species = 'Roach'),
    cow_perch_pike_pred_int %>% rename(individual_ID = Perch_ID) %>% mutate(Species = 'Perch')
  ) %>%
  dplyr::select(individual_ID, Species, Treatment, everything()) 

# Save the combined predation event data
saveRDS(cow_pred_events, paste0(enc_path, "cow_pred_events.rds"))
...
#remove rows for same individual's where we know the id of the pike that predated it
cow_roach_pike_pred_int <- cow_roach_pike_pred_int[-c(1, 2, 4, 5, 6), ]
...
roach_pike_encounter_sum_filt <- 
  ...
  distinct()  # Remove duplicate rows to ensure one entry per roach
...
# Add information about whether the roach were found alive at the end of the experiment
roach_pike_encounter_sum_filt <- 
  roach_pike_encounter_sum_filt %>%
  left_join(post_size_cols, by = c("Roach_ID" = "individual_ID"))
...
# pred_cols <- cow_perch_pike_pred_int %>%
#   filter(likely_predated == '1') %>%
#   dplyr::select(perch_ID, likely_predated)
# 
# perch_pike_encounter_sum_unfilt <- perch_pike_encounter_sum_unfilt %>%
...
# > 4.3. cow encounter summary ####

# Combine Roach-Pike and Perch-Pike encounter data into a single summary table of predation events in cow lake.
cow_pred_encounter_summary <- 
  rbind(
...
saveRDS(cow_pred_encounter_summary, paste0(enc_path, "cow_pred_encounter_summary.rds"))
...
pred_id_dist <- cow_pike_roach_distances_df %>% 
...
roach_pike_dist_plot <- ggplot(F59719_dist_avg, aes(x = date)) +
  ...
  theme(
    ...
    plot.sucowitle = element_text(size = 10),
...
p <- ggplot() + 
  geom_sf(data = cow_polygon, fill = NA, color = "black", linewidth = 1) +
  ...
  labs(
    ...
    sucowitle = '{ifelse(format(as.POSIXct(frame_along), "%Y-%m-%d") >= "2022-10-08" & 
                       format(as.POSIXct(frame_along), "%Y-%m-%d") <= "2022-10-21", 
                       "Roach predated!", 
                       "")}',
    x = "",
    y = ""
  ) +  # Show the date in the title and conditional sucowitle
  ...
  theme(
    ...
    plot.sucowitle = element_text(face = 'bold', color = 'red', size = 24, hjust = 0.5),  # Style for sucowitle
...
# 
# # Reproject Roach telemetry objects to match the Pike projection
# projection(roach_cow_tel) <- projection(pike_cow_tel)
# projection(roach_cow_ctmm_fits) <- projection(pike_cow_ctmm_fits)
# 
# 
# combined_telemetry <- c(roach_cow_tel["F59701"], pike_cow_tel["F59885"])
# combined_ctmm <- c(roach_cow_ctmm_fits["F59701"], pike_cow_ctmm_fits["F59885"])
# 
# 
