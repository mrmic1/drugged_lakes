...
### LOAD DATA ###
cow_filt_data <- readRDS(paste0(filtered_data_path, '02_lake_cow_sub.rds'))

#---------------------------------------------------------------------------------#

#Species duplicated in dataframe, need to fix
cow_filt_data <- cow_filt_data[,-6]
...
irreg_indiv <- daily_sampling_sum %>% 
  ...
  filter(poor_tracking_day == 1 & n_poor_tracking_days > 8)
...
save_as_docx(individual_irregular_sample_table, 
             path = paste0(save_tables_path, "cow_ID_day_locs_w_irregular_sampling.docx"))
...
save_as_docx(summary_data, 
             path = paste0(save_tables_path, "cow_ID_poor_tracking_summary.docx"))
...
#SAVE
saveRDS(summary_data, paste0(filtered_data_path, "cow_ID_irreg_sampling.rds"))
saveRDS(cow_filt_data, paste0(filtered_data_path, "03_lake_cow_sub.rds"))
saveRDS(daily_sampling_sum, paste0(filtered_data_path, "cow_daily_tracking_ID_stats"))
