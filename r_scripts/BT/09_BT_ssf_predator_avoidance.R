# ================================================================ #
# DYNAMIC SSF PREDATOR AVOIDANCE ANALYSIS - BT             #
# ================================================================ #
...
dir.create(paste0(ssf_path, "BT_perch/"), recursive = TRUE, showWarnings = FALSE)
dir.create(paste0(ssf_path, "BT_roach/"), recursive = TRUE, showWarnings = FALSE)
...
cat("Pike telemetry columns:", names(pike_BT_tel[[1]]), "\n")
...
# Convert pike
pike_df <- bind_rows(
  lapply(names(pike_BT_tel), function(id) {
    ctmm_tel_to_utm(pike_BT_tel[[id]], id_label = id)
  })
)

# Convert perch (with treatment column)
perch_df <- bind_rows(
  lapply(names(perch_BT_tel), function(id) {
    tel <- perch_BT_tel[[id]]
...
roach_df <- bind_rows(
  lapply(names(roach_BT_tel), function(id) {
    tel <- roach_BT_tel[[id]]
    ctmm_tel_to_utm(tel, id_label = id,
                    extra_cols = list(treatment = unique(tel$treatment)))
  })
...
perch_ssf_results <- lapply(perch_ids, function(pid) {
  ...
  if (!is.null(model)) {
    saveRDS(model, paste0(ssf_path, "BT_perch/", pid, "_ssf.rds"))
  }
...
saveRDS(perch_ssf_results, paste0(ssf_path, "BT_perch/perch_ssf_results.rds"))
perch_ssf_results <- readRDS(paste0(ssf_path, "BT_perch/perch_ssf_results.rds"))
...
roach_ssf_results <- lapply(roach_ids, function(rid) {
  ...
  if (!is.null(model)) {
    saveRDS(model, paste0(ssf_path, "BT_roach/", rid, "_ssf.rds"))
  }
...
saveRDS(roach_ssf_results, paste0(ssf_path, "BT_roach/roach_ssf_results.rds"))
roach_ssf_results <- readRDS(paste0(ssf_path, "BT_roach/roach_ssf_results.rds"))
...
# Save
write_csv(perch_ssf_coefs, paste0(ssf_path, "BT_perch/perch_ssf_coefs.csv"))
perch_ssf_coefs <- read_csv(paste0(ssf_path, "BT_perch/perch_ssf_coefs.csv"))

write_csv(roach_ssf_coefs, paste0(ssf_path, "BT_roach/roach_ssf_coefs.csv"))
roach_ssf_coefs <- read_csv(paste0(ssf_path, "BT_roach/roach_ssf_coefs.csv"))
...
plot_ssf_coefs <- function(coef_df, species_label, y_limits = c(-3, 2.5)) {
...
ggsave(paste0(fig_path, "perch_ssf_pike_od_BT.pdf"), perch_ssf_plot,
       width = 8, height = 7, units = "cm", dpi = 300, device = "pdf")
ggsave(paste0(fig_path, "roach_ssf_pike_od_BT.pdf"), roach_ssf_plot,
       width = 8, height = 7, units = "cm", dpi = 300, device = "pdf")
