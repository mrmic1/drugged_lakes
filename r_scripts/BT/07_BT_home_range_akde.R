#===============================================================================
# BT LAKE - FISH HOME RANGE ANALYSIS WITH AKDE AND BOUNDARY CORRECTION
#===============================================================================
#
# PURPOSE:
...
dir.create(paste0(akde_path, "BT_pike_akdes/"),  showWarnings = FALSE)
dir.create(paste0(akde_path, "BT_perch_akdes/"), showWarnings = FALSE)
dir.create(paste0(akde_path, "BT_roach_akdes/"), showWarnings = FALSE)
...
for(species in c("perch", "roach", "pike")) {
  tel  <- get(paste0(species, "_BT_tel"))
  fits <- get(paste0(species, "_BT_ctmm_fits"))
...
run_species_akde <- function(tel_list,
  ...
  saveRDS(akde_list, paste0(akde_save_path, tolower(species_name), "_BT_akdes.rds"))
...
pike_results <- run_species_akde(
  tel_list      = pike_BT_tel,
  fits_list     = pike_BT_ctmm_fits,
  sp_data       = BT_sp_data,
  control_idx   = 1:3,
  exposed_idx   = 4:6,
  species_name  = "Pike",
  akde_save_path = paste0(akde_path, "BT_pike_akdes/"),
  min_N         = 3
)

pike_BT_akdes  <- pike_results$akdes
...
perch_results <- run_species_akde(
  tel_list      = perch_BT_tel,
  fits_list     = perch_BT_ctmm_fits,
  sp_data       = BT_sp_data,
  control_idx   = 1:15,
  exposed_idx   = 16:30,
  species_name  = "Perch",
  akde_save_path = paste0(akde_path, "BT_perch_akdes/"),
  min_N         = 3
)
...
roach_results <- run_species_akde(
  tel_list      = roach_BT_tel,
  fits_list     = roach_BT_ctmm_fits,
  sp_data       = BT_sp_data,
  control_idx   = 1:13,
  exposed_idx   = 14:26,
  species_name  = "Roach",
  akde_save_path = paste0(akde_path, "BT_roach_akdes/"),
  min_N         = 3
)

roach_BT_akdes <- roach_results$akdes
