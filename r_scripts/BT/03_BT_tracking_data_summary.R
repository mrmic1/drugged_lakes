...
#telemetry objects
pike_lake_BT_tel = readRDS(paste0(telem_path, 'pike_BT_tel.rds'))
perch_lake_BT_tel = readRDS(paste0(telem_path, 'perch_BT_tel.rds'))
roach_lake_BT_tel = readRDS(paste0(telem_path, 'roach_BT_tel.rds'))
...
# mean: 305532 - sd: 190241 = 115291
irreg_indiv <- positions_sum %>% 
  filter(n_positions < 115291 | n_missing_dates >= 10 | n_breaks >= 2)
...
save_as_docx(daily_irregular_sample_table, 
             path = paste0(save_tables_path, "BT_ID_irregular_sampling_summary.docx"))
...
#SAVE
saveRDS(summary_data_test, paste0(filtered_data_path, "BT_ID_irreg_sampling.rds"))
