...
#saveRDS(lake_BT_filt_data, paste0(data_filter_path, "lake_BT_final_filt_data.rds"))
...
lake_BT_filt_data$time_diff <- as.numeric(round(lake_BT_filt_data$time_diff, digits = 3))

#check time_diff
head(lake_BT_filt_data %>% 
...
lake_BT_all_ctmm_fits <- list(Pike_fits = pike_lake_BT_ctmm_fits, 
                              Perch_fits = perch_lake_BT_ctmm_fits, 
                              Roach_fits = roach_lake_BT_ctmm_fits)
...
for (species in species_list) {
  ...
  individuals <- names(lake_BT_all_ctmm_fits[[species]])
  
  # Loop through each individual in the species
  for (ID in individuals) {
    # Extract the summary object
    summary_obj <- summary(lake_BT_all_ctmm_fits[[species]][[ID]])
...
#save_as_docx(missing_dates_table, 
              path = paste0(save_tables_path, "lake_BT_fish_w_missing_dates.docx"))
...
save_as_docx(positions_sum_table, 
             path = paste0(save_tables_path, "lake_BT_fish_locations_sum.docx"))

#16/66 individuals had some sort of missed tracking

lake_BT_filt_data <- merge(lake_BT_filt_data, 
...
#add column to lake_BT_filt_data flagging irregular individuals
...
save_as_docx(individual_irregular_sample_table, 
             path = paste0(save_tables_path, "lake_BT_individual_irregular_sampling.docx"))
...
reduced_data <- lake_BT_filt_data %>%
...
save_as_docx(daily_irregular_sample_table, 
             path = paste0(save_tables_path, "lake_BT_irregular_sampling.docx"))
...
save_as_docx(lake_BT_species_positions_sum, 
                            path = paste0(save_tables_path, "lake_BT_species_location_summary_MS.docx"))
