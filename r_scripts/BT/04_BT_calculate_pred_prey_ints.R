# ------------------------------------------------## 
# Exploring predator-prey interactions - Lake BT ###
# ------------------------------------------------##
...
# saveRDS(roach_pike_encounter_sum, paste0(enc_path, "BT_roach_pike_encounter_summary_unfiltered.rds"))
# saveRDS(perch_pike_encounter_sum, paste0(enc_path, "BT_perch_pike_encounter_summary_unfiltered.rds"))
...
prey_post_size_cols <- 
  post_biometrics %>%
  filter(Lake == 'BT', Species == 'Roach'| Species == 'Perch') %>%
...
BT_prey_pike_interactions <- 
  BT_prey_pike_interactions %>%
  left_join(BT_prey_max_encounter_dates %>%
...
saveRDS(BT_prey_max_encounter_dates, paste0(enc_path, "BT_prey_daily_max_pred_encounters.rds"))

# Save the suspected predation event data
saveRDS(BT_prey_pike_interactions, paste0(enc_path, "BT_suspected_predation_events.rds"))
...
saveRDS(BT_prey_total_encounter_summary, paste0(enc_path, 'BT_prey_total_encounter_summary_unfiltered.rds'))
saveRDS(BT_prey_daily_encounter_summary, paste0(enc_path, 'BT_prey_daily_encounter_summary_unfiltered.rds'))
...
BT_daily_encounter_summary_prey_not_found$Date <- as.Date(BT_daily_encounter_summary_prey_not_found$Date)
...
merged_df <- merge(BT_daily_encounter_summary_prey_not_found, prey_not_found_cols, 
...
BT_total_encounter_summary_prey_not_found <-
  BT_daily_encounter_summary_prey_not_found %>% 
  group_by(Prey_ID) %>% 
  summarise(Species = first(Species),
...
BT_daily_irregular_sample_table <- 
  flextable(BT_total_encounter_summary_prey_not_found) %>% 
...
save_as_docx(BT_daily_irregular_sample_table, 
             path = paste0(save_tables_path, "BT IDs not found encounter and tracking summary.docx"))

saveRDS(BT_total_encounter_summary_prey_not_found, paste0(enc_path, 'BT_total_encounter_summary_for_prey_not_found_unfiltered.rds'))
...
write.xlsx(BT_total_encounter_summary_prey_not_found, file = paste0(save_tables_path, "BT_IDs_not_found_total_encounter_summary.xlsx"))
...
prey_encounters_sum_IDs_not_found <- readRDS(paste0(enc_path, "BT_total_encounter_summary_for_prey_not_found_unfiltered.rds"))
