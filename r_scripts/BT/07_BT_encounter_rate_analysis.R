...
paths <- list(
  ...
  filtered   = "./data/tracks_filtered/BT/",
  mortality  = "./data/encounters/suspected_mortality_updated.xlsx",
  figures    = "./figures/BT/encounter_analysis/",
  tables     = "./tables/BT/encounter_analysis/"
)
...
roach_pike_distances_df <- readRDS(
  paste0(paths$encounters, "BT_pike_roach_distances_tiered_df.rds")
)
perch_pike_distances_df <- readRDS(
  paste0(paths$encounters, "BT_pike_perch_distances_tiered_df.rds")
)

## 2.2 Load metadata ----
BT_filt_data <- readRDS(
  paste0(paths$filtered, "05_BT_sub.rds")
)

# Extracting ID, treatment and weight information
BT_meta <- BT_filt_data %>%
...
BT_death_dates <- mortality_preds %>%
  filter(lake == "BT") %>%
...
print(table(BT_death_dates$species,
            BT_death_dates$revised_suspected_mortality))

## 3.2 Filter roach-pike distance data ----
roach_pike_distances_df <- roach_pike_distances_df %>%
  left_join(
    BT_death_dates %>% select(individual_ID, death_date),
...
enc_roach_total <- roach_pike_distances_df %>%
  ...
  left_join(BT_meta, by = "individual_ID") %>%
...
## 4.5.2 Load pre-filtered metadata (04_BT_sub.rds) ----
BT_pre_data <- readRDS(
  paste0(paths$filtered, "04_BT_sub.rds")
)

BT_pre_meta <- BT_pre_data %>%
  select(individual_ID, treatment, weight) %>%
  distinct()
...
enc_roach_pre <- roach_pike_raw %>%
  ...
  left_join(BT_pre_meta, by = "individual_ID") %>%
  filter(!is.na(treatment)) %>%
  mutate(
    Species        = "Roach",
    rate_high_conf = total_encounters_high_conf / n_days,
...
message("\nPre-filter encounter rate summary — BT:")
...
#       GPS error for BT: combined error = sqrt(2) * 0.379m = 0.536m,
#       95% CI ~1.07m; threshold set at 1.4m for cross-lake comparability).
#
...
#Lake BT roach had around 5.4 encounters with pike per day
#Big difference compared to muddyfoot which had 57.1
#Control roach had slightly more than exposed roach


print(pairs(emm_roach_hc))
#Control roach have 16% more encounters per day.
...
ggsave(file.path(paths$figures, "BT_encounter_rates_high_conf.pdf"),
...
ggsave(file.path(paths$figures, "encounter_rates_combined.pdf"),
...
# =================================================================-
# Key differences from muddyfoot: same model structure, BT-specific
# encounter thresholds baked into encounter_high_conf column (<=1.4m).
# Single-lake analysis (BT), so lake is not a factor.

## 6.1 Prepare hourly datasets ----
# Experiment start date for BT: 2022-09-26.
roach_h <- roach_hourly %>%
  mutate(
    exp_day = as.integer(
      as.duration(as.POSIXct(paste(Date, "00:00:00")) -
                    as.POSIXct("2022-09-26")) / lubridate::ddays(1)
    ),
    treatment     = factor(treatment, levels = c("Control", "Mix")),
    individual_ID = factor(individual_ID),
...
summary(roach_mod)
#Roach had less encounters overtime
#strong diel rhythmms
#exposed curves slightly less wiggly than control curves
anova.gam(roach_mod)

print_gamm_results(roach_mod, lake = "BT", species = "Roach")
...
ggsave(file.path(paths$figures, "BT_roach_diel_encounter_figure.pdf"),
...
summary(perch_mod)
#perch encounters decreased overtime

anova.gam(perch_mod)

print_gamm_results(perch_mod, lake = "BT", species = "Perch")
...
print(perch_diel_plot)

ggsave(file.path(paths$figures, "BT_perch_diel_encounter_figure.pdf"),
...
ggsave(file.path(paths$figures, "BT_combined_diel_encounter_overview.pdf"),
...
cat("Encounter Rate Analysis — BT — Session Info\n")
