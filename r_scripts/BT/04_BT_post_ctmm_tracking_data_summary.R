#################################################################################
### TRACKING DATA QUALITY ANALYSIS - LAKE BT
...
# Output directories
save_tables_path <- "./tables/BT/"
save_figures_path <- "./figures/BT/"

## 1.4 Load Data ----
BT_filt_data <- readRDS(paste0(filtered_data_path, "03_BT_sub.rds"))

## 1.5 Define Study Period ----
date_range <- range(BT_filt_data$Date, na.rm = TRUE) #"2022-09-26" "2022-10-29"
...
print(head(BT_filt_data %>% select(timestamp, time_diff), n = 20))
...
save_as_docx(
  positions_sum_table,
  path = paste0(save_tables_path, "BT_ID_positions_summary_pre_predation_event_filtering.docx")
)
...
daily_sampling_sum <- BT_filt_data %>%
  group_by(individual_ID, Date) %>%
  summarise(
    Species = first(Species),
    Treatment = first(Treatment),
...
print(species_day_positions_stats)
...
ggsave(
  filename = "BT_species_daily_positions_histogram.jpg",
...
POOR_TRACKING_THRESHOLD <- 8.5  # Days with poor tracking
...
save_as_docx(
  ...
  path = paste0(save_tables_path, "BT_ID_irregular_tracking_summary.docx")
)
