...
# 95% CI: ~1.4m
params <- list(
  # GPS error for BT lake (from UERE analysis)
  gps_error_sd = 0.379,  # Updated from 0.5m to 0.379m
  
  # Combined GPS error for prey+predator: √(0.379² + 0.379²) = 0.536m
  # 95% CI: ~1.4m (using 2.6σ for 95% confidence)
  
  # Tiered distance thresholds (meters) - UPDATED FOR BT
  strike_distance = 0.45,              # Pike max strike distance (unchanged)
  high_confidence_threshold = 1.4,     # ≤1.4m: Within 95% combined GPS error (≈2.6 × 0.536)
  ...
  # Mortality detection settings - UPDATED FOR BT
  gps_error_sd = 0.379,                # BT-specific GPS error (meters)
...
message("Tiered approach based on GPS error (σ = 0.379m per fish, BT lake):")
...
roach_pike_distances_df <- readRDS(paste0(paths$encounters, "BT_pike_roach_distances_tiered_df.rds"))
perch_pike_distances_df <- readRDS(paste0(paths$encounters, "BT_pike_perch_distances_tiered_df.rds"))
...
saveRDS(total_encounters_ID, paste0(paths$encounters, "BT_total_encounters_ID.rds"))
...
#Save workbook
saveWorkbook(wb,
             file.path(paths$tables, "BT_predation_analysis_complete.xlsx"),
             overwrite = TRUE)
...
message("\nKey files to review:")
message("  - BT_predation_analysis_complete.xlsx")
