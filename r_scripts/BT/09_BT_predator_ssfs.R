# ============================================================ #
# PREDATOR SPACE USE AVOIDANCE - RSF & HR OVERLAP - BT #
# ============================================================ #
...
BT_polygon <- st_read(paste0(polygon_path, "BT_polygon.gpkg"))  # adjust filename as needed

# Receiver and habitat locations
mud_rec_locs_kml <- paste0(rec_data_path, "BT_rec_hab_locations.kml")
...
pred_value_plot <- ggplot(predator_ud_df, aes(x = x, y = y)) +
  ...
  geom_sf(data = BT_polygon_tpeqd, fill = NA,
...
writeRaster(predator_ud_raster,
            paste0(rsf_path, "BT_predator_ud_raster.tif"),
            overwrite = TRUE)
...
perch_control_tel   <- perch_BT_tel[perch_treatments == "Control"]
perch_exposed_tel   <- perch_BT_tel[perch_treatments == "Mix"]
perch_control_akdes <- perch_BT_akdes[perch_treatments == "Control"]
perch_exposed_akdes <- perch_BT_akdes[perch_treatments == "Mix"]

# Roach
roach_treatments <- sapply(roach_BT_tel, function(tel) unique(tel$treatment))
print(roach_treatments)

roach_control_tel   <- roach_BT_tel[roach_treatments == "Control"]
roach_exposed_tel   <- roach_BT_tel[roach_treatments == "Mix"]
roach_control_akdes <- roach_BT_akdes[roach_treatments == "Control"]
roach_exposed_akdes <- roach_BT_akdes[roach_treatments == "Mix"]
...
for (i in seq_along(perch_control_tel)) {
  ...
  rsf_perch_control_list[[id]] <- tryCatch({
    ...
    saveRDS(model, paste0(rsf_path, "BT_perch/", id, "_predator_rsf.rds"))
...
rsf_perch_control_list <- fit_rsf_parallel(
  ...
  save_dir    = paste0(rsf_path, "BT_perch/")
)

verify_rsf_list(rsf_perch_control_list, "Perch - Control")

saveRDS(rsf_perch_control_list, paste0(rsf_path, "BT_perch/pred_rsf_perch_control_list.rds"))
...
saveRDS(rsf_perch_exposed_list, paste0(rsf_path, "BT_perch/pred_rsf_perch_exposed_list.rds"))
...
rsf_roach_control_list <- fit_rsf_parallel(
  ...
  risk_raster = predator_ud_raster,
  save_dir    = paste0(rsf_path, "BT_roach/")
)
saveRDS(rsf_roach_control_list, paste0(rsf_path, "BT_roach/pred_rsf_roach_control_list.rds"))
...
saveRDS(rsf_roach_exposed_list, paste0(rsf_path, "BT_roach/pred_rsf_roach_exposed_list.rds"))
...
# rsf_perch_control_list <- readRDS(paste0(rsf_path, "BT_perch/pred_rsf_perch_control_list.rds"))
# rsf_perch_exposed_list <- readRDS(paste0(rsf_path, "BT_perch/pred_rsf_perch_exposed_list.rds"))
# rsf_roach_control_list <- readRDS(paste0(rsf_path, "BT_roach/pred_rsf_roach_control_list.rds"))
# rsf_roach_exposed_list <- readRDS(paste0(rsf_path, "BT_roach/pred_rsf_roach_exposed_list.rds"))
...
ggsave(paste0(fig_path, "perch_predator_rsf_BT.png"), perch_rsf_plot,
       width = 8, height = 8, units = "cm", dpi = 300, device = "png")

ggsave(paste0(fig_path, "roach_predator_rsf_BT.png"), roach_rsf_plot,
       width = 8, height = 8, units = "cm", dpi = 300, device = "png")
...
# Save overlap tables
write_csv(overlap_perch, paste0(rsf_path, "BT_perch/perch_pkde_overlap.csv"))
write_csv(overlap_roach, paste0(rsf_path, "BT_roach/roach_pkde_overlap.csv"))
